...
<head>
    ...
    <style>
        ...
        .weui-navbar__item{color: #ddd;border-bottom: 3px solid #fafafa}
...
<div class="weui-navbar">
    <c:forEach items="${destinationList}" var="destination" varStatus="status">
        <a id="${destination.id}" class="weui-navbar__item weui-grid">${destination.name}</a>
    </c:forEach>
</div>
<!--对应内容-->
...
//        });
        ...
        $(document).on("click",".weui-grid",function () {
            ...
            req = $.ajax({
                type:"post",
                url:"${ctx}/wechat/sort/ShowProductByDistInfo",
                data:{cityId:cityId,destinationId:destinationId},
                dataType:"json",
                ...
                success:function (data) {
                    ...
                    for(var i = 0;i<data.length;i++){
                        var _score = Number(Number(data[i].productAvgVote).toFixed(2));
                        str += "<div class='my-item'>" +
                            "<a href='${ctx}/wechat/WxProductDetail/getProductDetail?productId="+
                            data[i].id +"&cityId=" +cityId+ "'>" +
                            "<div class='weui-flex my-pic'>" +
                            "<div class='weui-flex__item'>" +
                            "<img src='/photo/product/"+ data[i].picUrl+ "'>" +
                            "</div></div><div class='my-title'>" + data[i].name + "</div>" + "</a>";
                        for (var j = 0;j < data[i].activities.length;j++){
                            ...
                            if (j <= 2){
                                str +=
                                    ...
                                    data[i].activities[j].id+"&productId="+data[i].id+"&cityId=" +cityId+
                                    "'>"+
