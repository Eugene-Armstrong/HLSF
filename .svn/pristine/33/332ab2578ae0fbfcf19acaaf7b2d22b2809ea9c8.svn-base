...
<div class="weui-cells weui-cells_form">
    <div class="weui-cell">
        ...
        <div class="weui-cell__bd">
            <input class="weui-input" id="title" type="text" placeholder="请输入游记标题(必填)" value="${travelNote.title}">
        </div>
    </div>
</div>
<!--游记图文内容-->
...
<script>
    ...
    $(function () {
        Edr.append('${travelNote.content}');
        //草稿状态为“未能通过”
        if(${travelNote.state == 2}){
            var reason = "${travelNote.remark}";
            if(reason.length == 0){
                $.alert("很抱歉本篇游记未能通过审核，如有疑问，欢迎咨询手机或微信18520148370，感谢您的理解与支持。"
                    ,"温馨提示");
            }else{
                $.alert(reason,"抱歉，游记未能通过审核");
            }
        }
        //原封面图
        $addCover.html('<img src="${ctx}/photo/TravelNote/${travelNote.picUrl}"'+
        ...
        var originalCover = $addCover.find("img").attr("src");
        ...
        $(document)
            ...
            .on("click","#save",function () {
                var isDisclaimed,
                    ...
                    current = $addCover.find("img").attr("src");
                ...
                }else if(originalCover == current){ //封面图不变
                    $.ajax({
                        url: "${ctx}/wechat/journey/editJourney",
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id:"${travelNote.id}",
                            picUrl:"${travelNote.picUrl}",
                            title:$title.val(),
                            mediaIds: [],
...
//                                Edr.append('<img src="'+localIds[i]+'" alt="">');
//                            }
